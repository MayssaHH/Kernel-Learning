...
from data.generators.professor.monni_simulated_cauchy import generate_professor_split_cauchy
...
def run_experiement(
    ...
    """
    Test 11: same protocol as test9, but data comes from Cauchy heavy-tailed generator.

    Relative to test7:
    - Uses larger default signal structure (pc, p1, p2).
    - Keeps pn stress-test exactly the same idea.
    - Optionally clips heavy tails via cauchy_clip for numerical stability.
    ...
    results_dir = Path(results_root) / f"{run_uuid}_test11_professor_data_cauchy_shuffled_types"
    if save_results:
    ...
    for pn in pn_values:
            ...
            for epoch in range(epochs):
                ...
                if epoch % 100 == 0:
                    print(
                        ...
                        f"Epoch {epoch:>3} | Archi + All RBF alignment loss: {loss:.6f}"
                    )
            ...
            all_runs.append(
                {
                    ...
                    "permutation": permutation.tolist(),
                    "feature_types_after_shuffle": shuffled_feature_types,
                    "archi_all_rbf_accuracy": float(archi_acc),
        ...
        summary_rows.append(
            {
                "pn": int(pn),
                "mean_archi_all_rbf_accuracy": float(torch.tensor(archi_acc_runs).mean().item()),
                "std_archi_all_rbf_accuracy": float(torch.tensor(archi_acc_runs).std(unbiased=False).item()),
    ...
    plt.plot(x, [row["mean_archi_all_rbf_accuracy"] for row in summary_rows], marker="o", label="Archi + All RBF")
    ...
    plt.title("Test 11 (Professor Cauchy Data, Shuffled Features): Accuracy vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    plt.title("Test 11: Signal/Noise Alpha Mass vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    plt.xlabel("Number of Pure Noise Predictors (pn)")
    plt.ylabel("Mean Alpha Mass")
    plt.title("Test 11: Alpha Mass by Feature Type vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    manifest = {
        ...
        "config": {
            "experiment_name": "test11_professor_data_cauchy_shuffled_types",

...
def run_experiement(
    ...
    """
    Test 7: professor-style data protocol.
    ...
    results_dir = Path(results_root) / f"{run_uuid}_test7_professor_data"
    if save_results:
    ...
    plt.title("Test 7 (Professor Data): Accuracy vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    plt.title("Test 7 (Professor Data): Alpha Mass vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    manifest = {
        ...
        "config": {
            "experiment_name": "test7_professor_data",

...
from kernel_learning import AlignmentLoss, KernelNetwork, LinearSubKernel, ManualGradientTrainer
...
def run_experiement(
    ...
    """
    Test 10: same as test9, but architecture under test is all Linear sub-kernels.

    Relative to test7:
    - Uses larger default signal structure (pc, p1, p2).
    - Keeps pn stress-test exactly the same idea.
    - Optionally shuffles feature columns (same permutation for all iterations of a given pn),
      and tracks feature types so alpha plots are color-coded correctly.
    """
    pn_values = pn_values or [5, 10, 20, 50, 100]
    run_uuid = run_uuid or str(uuid.uuid4())

    results_dir = Path(results_root) / f"{run_uuid}_test10_professor_data_shuffled_types_all_linear"
    if save_results:
    ...
    for pn in pn_values:
        ...
        for i in range(iter_count):
            ...
            model_archi_linear = KernelNetwork(
            ...
            for epoch in range(epochs):
                ...
                if epoch % 100 == 0:
                    print(
                        ...
                        f"Epoch {epoch:>3} | Archi + All Linear alignment loss: {loss:.6f}"
                    )

            archi_acc = evaluate_kernel_model_with_krr(
                kernel_model=model_archi_linear,
                X_train=X_train,
                y_train=y_train,
                X_test=X_test,
                y_test=y_test,
            ...
            alphas = model_archi_linear._get_alphas().detach().cpu()
            ...
            all_runs.append(
                {
                    ...
                    "permutation": permutation.tolist(),
                    "feature_types_after_shuffle": shuffled_feature_types,
                    "archi_all_linear_accuracy": float(archi_acc),
            ...
            print(
                ...
                f"Archi + All Linear: {archi_acc:.4f}, "
        ...
        summary_rows.append(
            {
                "pn": int(pn),
                "mean_archi_all_linear_accuracy": float(torch.tensor(archi_acc_runs).mean().item()),
                "std_archi_all_linear_accuracy": float(torch.tensor(archi_acc_runs).std(unbiased=False).item()),
        ...
        legend_order = ["shared", "class_minus_only", "class_plus_only", "noise"]
        legend_handles = []
        for feature_type in legend_order:
    ...
    plt.plot(x, [row["mean_archi_all_linear_accuracy"] for row in summary_rows], marker="o", label="Archi + All Linear")
    ...
    plt.title("Test 10 (Professor Data, Shuffled Features): Accuracy vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    plt.title("Test 10: Signal/Noise Alpha Mass vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    plt.xlabel("Number of Pure Noise Predictors (pn)")
    plt.ylabel("Mean Alpha Mass")
    plt.title("Test 10: Alpha Mass by Feature Type vs Pure Noise Predictors")
    plt.ylim(0.0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend()
    ...
    manifest = {
        ...
        "config": {
            "experiment_name": "test10_professor_data_shuffled_types_all_linear",

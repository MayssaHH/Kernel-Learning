...
def _build_split_payloads(
    ...
    if protocol == "paper_strict":
        if n_splits != 1:
            print(f"[test15] protocol=paper_strict uses exactly 1 split. Ignoring n_splits={n_splits}.")
...
def _train_architecture(
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            print(
                f"[test15] [split={split_idx}/{effective_n_splits}] "
...
def run_experiement(
    ...
    results_dir = Path(results_root) / f"{run_uuid}_test15_alpha_pruning_{dataset_name}"
    if save_results:
    ...
    print(
        f"[test15] dataset={dataset_name} | protocol={protocol} | "
    ...
    local_model_keys = [ARCH_TO_MODEL_KEY[a] for a in architectures] + ["full_vector_rbf", "full_vector_linear"]
    ...
    for split_idx, X_train_np, X_test_np, y_train_np, y_test_np in split_payloads:
        ...
        print(f"[test15] pass1 | {pass1_report}")
        print(f"[test15] pass2 | {pass2_report}")
    ...
    plt.ylabel("Accuracy (%)")
    plt.ylim(0.0, 100.0)
    plt.title(f"Test15 Before/After Pruning Accuracy | {dataset_name}")
    ...
    plt.title(f"Test15 Pass2 - Pass1 Delta | {dataset_name}")
    plt.grid(axis="y", alpha=0.25)
    ...
    summary = {
        ...
        "notes": {
            "paper_models_after_pruning": (
                "Paper models (EasyMKL/AverageMKL/CKA/SMKL) are reported as reference values only. "
                "They are not retrained inside test15 because their training implementations are not in this repo."
            )
        },
    }
    ...
    manifest = {
        ...
        "config": {
            "experiment_name": "test15_alpha_pruning_two_pass",
            "dataset_name": dataset_name,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test15: two-pass alpha-guided feature pruning benchmark on UCI datasets."
    )

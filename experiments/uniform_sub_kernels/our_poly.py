...
print("=" * 80)
print(f"MODULAR KERNEL NETWORK - EXAMPLE USAGE Uniform kernels (all {our}) ")
print("=" * 80)
...
model1 = KernelNetwork.from_uniform_kernels(
    kernel_class=PolynomialSubKernel,
    num_features=X.shape[1],
    kernel_params={'degree': DEGREE, 'c': C},
...
def my_kernel(X_1, X_2, alphas, degree=DEGREE, c=C):
    """
    Parameters:
    ...
    for i in range(X_1.shape[1]):
        K += alphas[i] * (np.outer(X_1[:, i], X_2[:, i]) + c) ** degree
    return K
...
clf_linear = SVC(kernel=choice_of_kernel, degree=DEGREE, coef0=C)
clf_custom = SVC(kernel=partial(my_kernel, alphas=alphas))

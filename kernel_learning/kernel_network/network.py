    ...
    def from_pattern(
        ...
        **network_kwargs
        ...
        Args:
            ...
            kernel_params: Dict mapping kernel names to their params #NOTE: here you must be careful to match the keys in this dictionary to the kernel names used in the pattern. For example, if your pattern includes 'linear' and 'rbf', your kernel_params should look like: {'linear': {'initial_sigma': 1.0}, 'rbf': {'initial_gamma': 0.5}}. If you pass a kernel class directly in the pattern, the name will be derived from the class name (e.g., LinearSubKernel -> 'linear'), so make sure to use that as the key in kernel_params if you want to specify parameters for that kernel (this is shown in this line of code kernel_name = kernel_class.__name__.lower().replace('subkernel', '')) make sure to match the keys accordingly. #NOTE: if you are not careful here this won't fail but it will give you warnings 
